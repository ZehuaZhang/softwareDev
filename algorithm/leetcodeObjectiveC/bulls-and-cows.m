...
NSString* getHint(NSString* secret, NSString* guess) {
  int missedCows[256], bulls = 0, cows = 0;
  for (int i = 0; i < secret.length; ++i) {
    if ([secret characterAtIndex:i] == [guess characterAtIndex:i]) {
      ++bulls;
    } else {
      if (missedCows[[secret characterAtIndex:i]]++ < 0) {
        ++cows;
      }
      if (missedCows[[guess characterAtIndex:i]]-- > 0) {
        ++cows;
      }
    }
  }
  return [NSString stringWithFormat:@"%d%@%d%@", bulls, @"A", cows, @"B"];
}
