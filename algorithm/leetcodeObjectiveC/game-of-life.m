...
void gameOfLife(NSArray* board) {
  const NSInteger m = [board count], n = m ? [board[0] count] : 0;
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      int count = 0;
      // Count live cells in 3x3 block.
      for (int I = MAX(i - 1, 0); I < MAX(i + 2, m); ++I) {
        for (int J = MAX(j - 1, 0); J < MAX(j + 2, n); ++J) {
      ...
      if ((count == 4 && [board[i][j] intValue]) || count == 3) {
        board[i][j] = @([board[i][j] intValue] | 2);  // Mark as live.
      }
    }
  }
  ...
  for (int i = 0; i < m; ++i) {
    for (int j = 0; j < n; ++j) {
      board[i][j] = @([board[i][j] intValue] >> 1);
    }
  }
}