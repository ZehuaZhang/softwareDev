...
public:
    NumMatrix(vector<vector<int>> &matrix) {
        ...
        for (int i = 0; i <= m; ++i) {
            _sums.emplace_back(n + 1, 0);
        }
        for (int i = 1; i <= m; ++i) {
            for (int j = 1; j <= n; ++j) {
                _sums[i][j] = _sums[i][j - 1] + _sums[i - 1][j] - _sums[i - 1][j - 1] + matrix[i - 1][j - 1];
            }
        }
    }

    int sumRegion(int row1, int col1, int row2, int col2) {
        return _sums[row2 + 1][col2 + 1] - _sums[row2 + 1][col1] - 
               _sums[row1][col2 + 1] + _sums[row1][col1];   
    }
