 ...
 * 
 * Excel(int H, char W): This is the constructor. The inputs represents the height and width of the Excel form. His a positive integer, range from 1 to 26. It represents the height.
...
public class Excel {
    Excel (int row, char col) {
        valuesBoard = new int[numbers][character - 'A' + 1];
        queriesMap = new HashMap<>();
    }

    public int get(int row, char col) {
        String key = new StringBuilder().append(row).append(col).toString();

        if (!queriesMap.containsKey(key)) {
            return valuesBoard[row - 1][col - 'A'];
        }
        return sum(row, col, queriesMap.get(key));
    }
    ...
    public void set(int row, char col, int value) {
        ...
        valuesBoard[row - 1][col - 'A'] = value;
    }

    private int sum(int row, char col, List<String> queries) {
        int sum = 0;
        for (String query : queries) {
            int indexOfDelimiter = query.indexOf(":");
            if (indexOfDelimiter < 0) {
                int queryRow = Integer.parseInt(query.substring(1));
                char queryCol = query.charAt(0);
                sum += get(queryRow, queryCol);
            } else {
                int queryRow1 = Integer.parseInt(query.substring(1, indexOfDelimiter));
                char queryCol1 = query.charAt(0);
                int queryRow2 = Integer.parseInt(query.substring(indexOfDelimiter + 2));
                char queryCol2 = query.charAt(indexOfDelimiter + 1);
                for (int i = queryRow1; i <= queryRow2; ++i) {
                    for (char j = queryCol1; j <= queryCol2; ++j) {
                        sum += get(i, j);
                    }
                }
            }
        }
        queriesMap.put(new StringBuilder().append(row).append(col).toString(), queries);
        return sum;
    }
