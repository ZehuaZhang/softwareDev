...
function maxVacationDays(flights: number[][], days: number[][]): number {
  ...
  for (let i = 1; i < weekCount; ++i) {
    for (let j = 0; j < cityCount; ++j) {
      ...
      if (prev >= 0) {
        dp[i][j] = prev + days[j][i];
      }
    }
  }
  const result = Math.max(...dp[weekCount - 1]);
  return result;
}
...
const expectedResultList = [12, 3, 21];

runTestCaseList(testInputListCollection, expectedResultList, maxVacationDays);
