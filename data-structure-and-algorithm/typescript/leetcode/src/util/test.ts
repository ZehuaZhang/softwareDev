...
function stringifyInputList(list: any[]) {
    return list
        .map(element => stringify(element))
        .join(", ");
}
...
export function runTestCaseList(inputCollection: inputCollection, expectedList: expectedResultList, handler: Function) {
    let passCount = 0;
    let failCount = 0;
    printSegmentLine();
