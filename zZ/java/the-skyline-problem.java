...
public class Solution {
    public List<int[]> getSkyline(int[][] buildings) {
        ...
        for (int[] building : buildings) {
            heights.add(new Integer[] {building[0], -building[2]});
            heights.add(new Integer[] {building[1], building[2]});
        }
        ...
        TreeMap<Integer, Intger> heightCount = new TreeMap<>();
        heightCount.put(0, 1);
        ...
        for (int[] height : heights) {
            if (height[1] < 0) {
                ...
                if (heightCount.get(height[1]) <= 0) {
                    heightCount.remove(height[1]);
                }
            }

            curr = heightCount.lastKey();
            if (curr != prev) {
