...
public class AutocompleteSystem   {
    AutocompleteSystem(List<String> terms, List<Integer> frequencies) {
        searchTermFrequencies = new HashMap<>();
        for (int i = 0; i < terms.size(); ++i) {
            createOrUpdate(terms.get(i), frequencies.get(i));
        }
        currentSearchTerm = new StringBuilder();
        maxSuggestionCount = 3;
    }

    public List<String> input(char c) {
        if (c == "#") {
            if (!currentSearchTerm.toString().isBlank()) {
                createOrUpdate(currentSearchTerm, 1);
            }
        }

        currentSearchTerm.append(c);

        PriorityQueue<Pair> queue = new PriorityQueue<>(new Comparator<Pair>() {
            public int compare(Pair pair1, Pair pair2) {
                if (pair1.frequency == pair2.frequency) {
                    return pair2.term.compareTo(pair1.term);
                }
        ...
        for (Map.Entry<String, Intger> termFrequency : searchTermFrequencies.entrySet()) {
            if (termFrequency.getKey().indexOf(currentSearchTerm.toString()) == 0) {
                queue.offer(new Pair(termFrequency.term, termFrequency.frequency));

                if (queue.size() > maxSuggestionCount) {
                    queue.poll();
                }
            }
        }
    ...
    private createOrUpdate(String term, int frequency) {
        int currFrequency = searchTermFrequencies.getOrDefault(term, 0);
        currFrequency += frequency;
        searchTermFrequencies.put(term, currFrequency);
    }

    private Map<String, Integer> searchTermFrequencies;
    private StringBuilder currentSearchTerm;
    ...
    private class Pair {
        ...
        Pair (String term, Integer frequency) {
            this.term = term;
