...
NSInteger lengthLongestPath(NSString* input) {
  NSMutableDictionary* pathLen = @{@0:@0}.mutableCopy;
  NSArray* paths = [input componentsSeparatedByString:@"\n"];
  
  NSInteger maxLen = 0;
  for (NSString* path in paths) {
    NSString* name = [path stringByReplacingOccurrencesOfString:@"\t" withString:@""];
    NSInteger depth = path.length - name.length;
    
    if ([name rangeOfString:@"."].length != NSNotFound) {
      maxLen = MAX(maxLen, [pathLen[@(depth)] intValue] + name.length);
    } else {
      pathLen[@(depth + 1)] = @([pathLen[@(depth)] intValue] + name.length + 1);    // update base len for next depth, 1 <=> '\'
    }
  }
  return maxLen;
}
